...
class fact_gen():
    def __init__(self, openai_key, serper_key, gen_model="gpt-3.5-turbo", eval_model="gpt-3.5-turbo",output_file=None, search=True):
    ...
    def zero_generate(self, topic, temp=1):
        ...
        #sentences = sent_tokenize(result_str)
    ...
    def retr_generate(self, topic, evidence, temp=1):
        self.search_time = 0
        self.openai_time = 0
        self.hf_time = 0
        min_tokens = 100
        ...
        if evidence:
            evidence = " ".join(evidence.split(" ")[:8000])
            filled_prompt = f"Context: {evidence}" + "\n\n" + f"Based on the provided context, tell me a bio of {topic}."
        else:
            filled_prompt = f"Tell me a bio of {topic}."
        
        chat_history = [     
            {"role": "system", "content": "You are a helpful assistant designed to help me produce accurate and factual texts based on the given prompt. Please generate only one sentence given the original prompt at a time. Don't ask anything."},
            {"role": "user", "content": filled_prompt}
        ]
        result_str = ""
        while len(result_str.split(" ")) < min_tokens:
        #print(filled_prompt)
            init_new_sent = self.sent_generate(chat_history, temp=0, max_tokens=128)
            chat_history.extend(
                    [{"role": "assistant", "content": init_new_sent}]
                )
            result_str += init_new_sent + " "
        return result_str, self.openai_time, self.search_time, self.hf_time
    ...
    def init_rag_generate(self, prompt, topic=None, temp=0.5, min_tokens=200, sent_max_tokens=128, ex_toler=0, discard=False, self_query=False):
       ...
       # if len(overall_extrinsic) > 0:
        ...
        if self.output_file:
            ...
            #print("openai_time:", self.openai_time, file=self.output_file)
            #print("spacy_time:", self.spacy_time, file=self.output_file)
            #rint("utils_time:", self.utils_time, file=self.output_file)
            with open(self.output_file, "a", buffering=1) as f:
    ...
    def rewrite(self, extrinsic_list, context, topic, max_tokens=64, temp=0, n=1):
        ...
        if self.gen_model_name == "gpt-3.5-turbo" or self.gen_model_name == "gpt-4":
            ...
            for dic in message:
                if dic["role"] == "system":
                    ...
                    #p#t.add_user_message(dic["content"])
    ...
    def revise(self, sent, intrinsic_list, context, topic, max_tokens=64, temp=0, n=1):
        #prompt = f"For the original text, which should be after the preceding context, please revise each wrong entity mentioned below, along with its associated wrong claim, based on the provided feedback.\n\nPreceding Context: {context}\n\nOriginal text: {sent}\n\n"
        if context:
    ...
    def gen_question(self, sent, topic, entity_set, temp=0, n=1):
        ...
        #prompt += 'Sentence: {}\nFor the above sentence about "{}", generate a yes/no question WITHOUT any pronouns that tests the correctness of "{}".\nQuestion: '
        for ent in entity_set:
            if ent:
                ...
                #with open(self.output_file, "a", buffering=1) as f:
                    #print(f"yes/no question: {filled_prompt}", file=f)
                    #f.close()
            ...
            if qdic["entities"][i]:
                ...
                if self.val_model_name == "gpt-3.5-turbo" or self.val_model_name == "gpt-4":
                    ...
                    for dic in history:
                        if dic["role"] == "user":
                            prompt = dic["content"]
                    atomic_response = self.hf_gen(prompt, "eval",  n=n, temp=temp, max_new_tokens=50)
                    atomic_fact = atomic_response[0]["generated_text"][len(prompt):].strip().replace('"', "").replace("'", "")
                    if "\n" in atomic_fact:
    ...
    def group_search(self, fact_dic, subject, self_query=False):
        ...
        for i in range(len(fact_dic["entities"])):
            ...
            if not self_query:
                ...
                history = [
                            #{"role": "system", "content": "You are a fact-checking assistant.  you will check if the claim is true or false given the provided evidence."},
                            {"role": "user", "content": filled_prompt}
                        ]
